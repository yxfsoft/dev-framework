...
## ADR-012: upgrade-project.py 重构计划（v3.0）

**决策**：将 `upgrade-project.py` 的重构列为后续计划项。

**原因**：
- 当前 upgrade-project.py 硬编码了 v2.6→v3.0 的迁移逻辑，随着版本演进会越来越难维护
- 需要引入版本化迁移机制（类似数据库 migration），每个版本升级对应一个迁移函数
- 当前阶段优先修复已确认的 15 个问题，重构工作延后

**替代方案**：在当前版本中直接重构。被拒：当前升级脚本功能正常，重构优先级低于质量修复。

**后果**：后续版本需投入时间重构升级机制，当前可更快发布。

---

## ADR-013: 轻量迭代模式 (lightweight) 定义（v3.0）

**决策**：定义轻量迭代模式的完整执行规则，包括启用条件、Phase 合并方式、合并 Agent 职责和记录义务。

**原因**：
- lightweight 模式此前仅标记为"实验性"，缺少完整的执行规则和启用条件
