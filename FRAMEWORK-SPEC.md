...
Claude Code 的自动上下文压缩通过以下机制保证不影响开发：

1. **磁盘为唯一真相源（P3）**：所有关键信息在产生时立即写入磁盘
2. **常驻校准协议**：项目 CLAUDE.md 中包含上下文校准指令（§八），
   该指令在压缩后仍存在于系统提示中，引导 Agent 在每个步骤前
   从磁盘文件校准状态
3. **任务内步骤记录**：task YAML 的 `current_step` 字段记录任务
   内部进度，压缩后可准确恢复到中断的步骤
4. **完整恢复工具**：`session-manager.py resume` 输出包含任务状态、
   检查点、决策日志、经验日志、git log、基线摘要的完整恢复摘要

压缩保护的核心策略是**"习惯驱动"而非"事件驱动"**——
不检测压缩是否发生，而是让 Agent 持续从磁盘校准状态，
使压缩对 Agent 行为透明。
